...
after_initialize do
  on(:topic_created) do |topic|
    if SiteSetting.private_template_tagger_enabled && topic.private_message?
      user = topic.user
      target_usernames = topic.allowed_users.pluck(:username).map(&:downcase) - [user.username.downcase]
      if target_usernames.include?(SiteSetting.private_template_tagger_username.downcase)
